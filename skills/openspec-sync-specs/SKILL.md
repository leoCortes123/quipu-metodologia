...
> Skill genérica (cualquier harness). Copia canónica del set openspec; la única
> diferencia con `adapters/<harness>/skills/` es la cabecera propietaria y la
> ortografía del disparador (`/opsx:*` vs `/opsx-*` según tu harness).
> Requiere el CLI `openspec`. Si tu harness no soporta skills, sigue
> `metodologia/workflows/` que describe lo mismo en checklist.
